...
class PrimitiveFunction(SchemeValue):
    ...
    def apply_step(self, args, evaluation):
        ...
        except (ZeroDivisionError, TypeError) as txt:
            raise SchemeError(txt.args[0])
...
class Evaluation:
    ...
    def do_call_form(self):
        ...
        p=self.expr.cdr
        while p.pairp():
            args.append(self.full_eval(p.car))
            p=p.cdr
...
def scm_read():
    ...
    if syntax == NUMERAL:
        ...
        "*** YOUR CODE HERE ***"
        items=scm_read()
        return scm_list(Symbol.string_to_symbol('quote'),items)
        return FALSE
    elif syntax == "(":
        return read_tail()
    else:
